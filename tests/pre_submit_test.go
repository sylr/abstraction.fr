...
func (w *testImportOrderingWalker) walk(path string, info os.FileInfo, _ error) error {
	...
	// 3 blocks total, if
	// 3 blocks: std + abstraction + 3rd party
	// 2 blocks: std + abstraction, std + 3rd party, abstraction + 3rd party
	// 1 block: std, abstraction, 3rd party
	// first entry in a block specifies the type (std, abstraction, 3rd party)
	// we want: std, abstraction, 3rd party
	...
	// if it:
	// contains strings abstraction.fr -> abstraction
	// contains dots -> 3rd
	// no dots -> std
	ip := [3]string{} // type per block, just string, either std, abstraction, 3rd
	for i := 0; i <= bl; i++ {
	...
	case 0:
		// we don't care
	case 1:
		if ip[0] == "std" && ip[1] == "abstraction" {
			break // OK
		}
		if ip[0] == "std" && ip[1] == "3rd" {
			break // OK
		}
		if ip[0] == "abstraction" && ip[1] == "3rd" {
			break // OK
		}
		absPath, _ := filepath.Abs(path)
		w.Errors = append(w.Errors, fmt.Errorf("import path in %q are not in the right order (std -> abstraction -> 3rd)", absPath))
	case 2:
		if ip[0] == "std" && ip[1] == "abstraction" && ip[2] == "3rd" {
			break // OK
		}
		absPath, _ := filepath.Abs(path)
		w.Errors = append(w.Errors, fmt.Errorf("import path in %q are not in the right order (std -> abstraction -> 3rd)", absPath))
...
func importtype(s string) string {
	if strings.Contains(s, "abstraction.fr") {
		return "abstraction"
	}
